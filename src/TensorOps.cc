...
// NOLINTBEGIN
#define LEANMT_TRANSPOSE_10_EXPLICIT(Type)                                    \
  template void transpose_10<Type>(const Type* in, size_t rows, size_t cols, \
                                   Type* out)
// NOLINTEND

LEANMT_TRANSPOSE_10_EXPLICIT(float);
LEANMT_TRANSPOSE_10_EXPLICIT(int);
LEANMT_TRANSPOSE_10_EXPLICIT(uint32_t);
LEANMT_TRANSPOSE_10_EXPLICIT(int8_t);
#undef LEANMT_TRANSPOSE_10_EXPLICIT
...
// NOLINTBEGIN
// matrix_multiply backed by ruy (the sole backend). Multiplies float
// matrices; the int8 weight multiply lives in qmm/Ruy.inl.cc.
