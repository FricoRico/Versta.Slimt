...
void Decoder::register_parameters(const std::string &prefix,
  ...
  // PrepareB-form alias of `Wemb`, named `Wemb_intgemm8` (the marian model-file
  // name for the legacy int8 format). For two-vocab models it's the alias of
  // `decoder_Wemb`, named `decoder_Wemb_intgemm8`. Register both — Io.cc only
  // emits whichever matches the model file.
...
void Transformer::load_parameters() {
  ...
  for (io::Item &item : items_) {
    ...
    if (target) {
      ...
      // alias, the marian int8-format name) covers both encoder input
      // embedding and the decoder's input/output projection. The Decoder holds
      // a reference to `decoder_embedding_`; mirror the encoder copy into it
      // below.
