...
// Explicit unrolls.
#define LEANMT_PRINT_NDARRAY_EXPLICIT(ScalarType)               \
  template std::ostream &print_ndarray(std::ostream &out,      \
                                       const ScalarType *data, \
                                       const std::vector<uint64_t> &dims)

LEANMT_PRINT_NDARRAY_EXPLICIT(float);
LEANMT_PRINT_NDARRAY_EXPLICIT(int);
LEANMT_PRINT_NDARRAY_EXPLICIT(int8_t);
LEANMT_PRINT_NDARRAY_EXPLICIT(uint32_t);

#undef LEANMT_PRINT_NDARRAY_EXPLICIT
...
#define LEANMT_TENSOR_FROM_FILE_EXPLICIT(type) \
  template Tensor tensor_from_file<type>(     \
      const std::string &fpath, const Shape &shape, const std::string &name)

#define LEANMT_QTENSOR_FROM_FILE_EXPLICIT(type, quant_type) \
  template std::tuple<Tensor, quant_type>                  \
  quantized_tensor_from_file<type, quant_type>(            \
      const std::string &fpath, const Shape &shape, const std::string &name)

LEANMT_TENSOR_FROM_FILE_EXPLICIT(float);
LEANMT_TENSOR_FROM_FILE_EXPLICIT(int);
LEANMT_TENSOR_FROM_FILE_EXPLICIT(int8_t);
LEANMT_TENSOR_FROM_FILE_EXPLICIT(uint32_t);

LEANMT_QTENSOR_FROM_FILE_EXPLICIT(int8_t, float);

#undef LEANMT_TENSOR_FROM_FILE_EXPLICIT
