...
void quantize(const float* input, float scale, Index rows, Index width,
  ...
  for (size_t i = 0; i < size; i++) {
    // Round half-to-even (nearbyintf, default FE_TONEAREST) to match marian's
    // `_mm*_cvtps_epi32` quantize rounding. roundf rounds
    // half-away-from-zero; the ±1 int8 disagreements on tie values shift the
    // alignment argmax at tail cross-attention knife-edges (the off-by-one
    // cascade).
...
std::vector<Tensor> affine_segmented(
...
Tensor select_columns(const Tensor& W,
...
Tensor dot(const Tensor& x, const Tensor& W, float a_quant,
...
void prepare_weight_transposed(const float* weights,
                                              int8_t* prepared,
...
void prepare_weight_quantized_transposed(const int8_t* input,
                                                        int8_t* output,
                                                        size_t rows,
                                                        size_t cols) {
...
Tensor prepare_bias(const Tensor& W, const Tensor& b,
                                   float a_quant, float b_quant,
                                   const std::string& name) {
  (void)W;
  (void)a_quant;
...
Tensor affine_with_prepared_bias(const Tensor& x,
                                                const Tensor& W,
                                                const Tensor& prepared_bias,
                                                float a_quant, float b_quant,
                                                const std::string& name) {
  return affine(x, W, prepared_bias, a_quant, b_quant, name);
}
